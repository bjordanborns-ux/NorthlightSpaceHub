const PROJECTS = [
  {
    ...
    badge: "Operator UI",
    time: "2–4 weeks",
    impact: "Faster state awareness with safer command entry",
    overview:
  ...
  },
  {
    ...
    badge: "Planning",
    time: "1–3 weeks",
    impact: "Reduced handover friction with operator-ready schedules",
    overview:
  ...
  },
  {
    ...
    badge: "Validation",
    time: "1–2 weeks",
    impact: "Cut review time 30% by surfacing actionable violations",
    overview:
...
function renderProjectCard(p) {
  ...
  const rows = [];
  if (p.impact) rows.push({ k: "Impact", v: p.impact });
  (p.highlights || []).slice(0, 1).forEach((h) => rows.push(h));
    ...
    .map(
      (h) => `
        <div class="glance-row">
          <span class="glance-k">${escapeHtml(h.k)}</span>
          <span class="glance-v">${escapeHtml(h.v)}</span>
        </div>
      `,
    )
    .join("");
  ...
  const tech = (p.tech || [])
    .slice(0, 3)
    .map((t) => `<span class="tag tag--tech">${escapeHtml(t)}</span>`)
    .join("");
  ...
  return `
    <article class="project" role="button" tabindex="0" aria-label="Open details for ${escapeHtml(p.title)}" data-id="${escapeHtml(
      ...
      <div class="project-glance" aria-label="At a glance">${glance}</div>
      <div class="project-tags" aria-label="Tags">${tags}</div>
      <div class="project-tech" aria-label="Tech preview">${tech}</div>
...
function openProjectModal(id) {
  ...
  const modalHighlights = [
    ...(p.impact ? [{ k: "Impact", v: p.impact }] : []),
    ...(p.highlights || []),
  ];
  highlights.innerHTML = modalHighlights.map(
      (h) => `
        <div class="highlight">
...
const HERO_HEADING_SEGMENTS = [
  { text: "Software projects built for ", accent: false },
  { text: "reliability", accent: true },
  { text: ", ", accent: false },
  { text: "clarity", accent: true },
  { text: ", and ", accent: false },
  { text: "ops readiness", accent: true },
  { text: ".", accent: false },
];

function buildHeroHeadingHtml(segments) {
  return segments
    .map((seg) => (seg.accent ? `<span class="accent">${seg.text}</span>` : seg.text))
    .join("");
}

function installHeroTypewriter() {
  const el = document.getElementById("hero-typewriter");
  if (!el) return;

  const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  if (reducedMotion) {
    el.innerHTML = buildHeroHeadingHtml(HERO_HEADING_SEGMENTS);
  ...
  const typeSpeed = 42;
  const pauseAfterPunctuation = 220;

  function schedule(next, delay = typeSpeed) {
    window.setTimeout(next, delay);
  }

  function tick() {
    if (segIndex >= HERO_HEADING_SEGMENTS.length) {
      el.classList.add("is-done");
      return;
    }

    const seg = HERO_HEADING_SEGMENTS[segIndex];

    if (charIndex === 0 && seg.accent) {
      html += '<span class="accent">';
    }

    html += seg.text[charIndex];
    charIndex++;
    ...
    const lastChar = seg.text[charIndex - 1];
    const delay = lastChar === "." || lastChar === "," ? pauseAfterPunctuation : typeSpeed;
    schedule(tick, delay);
  }
